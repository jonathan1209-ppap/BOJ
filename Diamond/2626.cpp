...
namespace cover_2d {
    ...
    bool in(const Circle& c, Point p) { 
                    ...
                    if (!in(c, p[j])) {
                        ...
                        for (int k = 0; k <= j; ++k) 
                            if (!in(ans, p[k])) {
                                ...
                                if (c.r < 0) 
                                    continue;
                                else if (a2 > 0 && (l.r < 0 || area2(pq, c.p - p[i]) > area2(pq, l.p - p[i]))) 
                                    l = c;
                                else if (a2 < 0 && (r.r < 0 || area2(pq, c.p - p[i]) < area2(pq, r.p - p[i]))) 
                                    r = c;
                            }
                        if (l.r < 0 && r.r < 0) 
                            c = ans;
...
int main() {
  fastio;
    int n;
    cin >> n;
    vector<cover_2d::Point> points(n);
    for (int i = 0; i < n; ++i) {
        double x, y;
        cin >> x >> y;
        points[i] = {x, y};
    ...
    cover_2d::Circle result = cover_2d::solve(points);
    cout << fixed << setprecision(3)
         << result.p.real() << " " << result.p.imag() << "\n" << result.r << "\n";
